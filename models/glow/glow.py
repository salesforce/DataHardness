...
from models.glow.coupling import Coupling, AdditiveCoupling
...
class Glow(nn.Module):
    ...
    def __init__(self, num_channels, num_levels, num_steps, input_shape=(3, 32, 32), q=[0.1 for x in range(10)]):
        ...
        self.flows = _Glow(in_channels=4 * input_shape[0],  # RGB image after squeeze
                           mid_channels=num_channels,
                           num_levels=num_levels,
                           num_steps=num_steps)
        #self.means = nn.Parameter(torch.zeros((len(q), *input_shape)))
        self.means = nn.Parameter(torch.randn((len(q), *input_shape)))
        self.cov_diag = nn.Parameter(torch.ones(input_shape))
        self.linproj = nn.Linear(np.prod(input_shape), len(q))
        self.q = q
    ...
    def forward(self, x, reverse=False):
        ...
        return x, sldj, self.linproj(x.view(x.size(0), -1))
...
class GlowAdditive(nn.Module):
    ...
    def __init__(self, num_channels, num_levels, num_steps, input_shape=(3, 32, 32), q=[0.1 for x in range(10)]):
        ...
        # Use bounds to rescale images before converting to logits, not learned
        self.register_buffer('bounds', torch.tensor([0.9], dtype=torch.float32))
        self.flows = _Glow_Additive(in_channels=4 * input_shape[0],  # RGB image after squeeze
                           mid_channels=num_channels,
                           num_levels=num_levels,
                           num_steps=num_steps)
        self.means = nn.Parameter(torch.randn((len(q), *input_shape)))
...
class _Glow_Additive(nn.Module):
    """Recursive constructor for a Glow model. Each call creates a single level.
    ...
    """
    def __init__(self, in_channels, mid_channels, num_levels, num_steps):
        super(_Glow_Additive, self).__init__()
        self.steps = nn.ModuleList([_FlowStep_Additive(in_channels=in_channels,
                                              mid_channels=mid_channels)
                                    for _ in range(num_steps)])

        if num_levels > 1:
            self.next = _Glow_Additive(in_channels=2 * in_channels,
                              mid_channels=mid_channels,
                              num_levels=num_levels - 1,
                              num_steps=num_steps)
        else:
...
class _FlowStep_Additive(nn.Module):
    def __init__(self, in_channels, mid_channels):
        super(_FlowStep_Additive, self).__init__()

        # Activation normalization, invertible 1x1 convolution, affine coupling
        self.norm = ActNorm(in_channels, return_ldj=True)
        self.conv = InvConv(in_channels)
        self.coup = AdditiveCoupling(in_channels // 2, mid_channels)
